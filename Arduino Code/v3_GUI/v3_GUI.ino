...
void doCompression(float force) {
  ...
  float timeDelay = (abs(trueStep)/mmPerSec) * 1000;  //1/(mmPerSec/trueStep); //time to do one step
  timeDelay -= trueStep * 7.5 * 1000;    //correct for the time it takes to turn the motor
  if (timeDelay > 0) {
    delay(timeDelay); 
  } else {
    Serial.println("NEGATIVE TIME DELAY");
  }
  ...
  if (Serial.available() > 0) {
    ...
    if (inByte == '~') { //stop reading
      ...
      if (incoming.substring(0, 3) == "RAT") {
        mmPerSec = incoming.substring(3, incoming.length()).toFloat();
      }
...
float measure() {
  ...
  if (newDataReady) {
    ...
    if (plotCount = 1000 && i != -1000) {
      Serial.println("DATA: ," + String(i) + "," + String(targetF) + "," + String(displacement) + "," + String(maxCompression) + "," + String(mmPerSec));
      plotCount = 0;
    } else {
      plotCount++;
    }
